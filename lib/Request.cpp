...
namespace Mongoose
{
    Request::Request(struct mg_connection *connection, struct mg_http_message *message, bool isMultipart):
        mIsValid(true),
        mIsMultipartRequest(isMultipart),
        mConnection(connection)
    {
        mMethod = std::string(message->method.ptr, message->method.len);
        mUrl = std::string(message->uri.ptr, message->uri.len);
        mQuerystring = std::string(message->query.ptr, message->query.len);

        for(int i = 0; i < MG_MAX_HTTP_HEADERS && message->headers[i].name.len != 0; i++)
        {
            mHeaders[message->headers[i].name.ptr] = message->headers[i].value.ptr;
        }

        if(!mIsMultipartRequest)
        {
            mBody = std::string(message->body.ptr, message->body.len);
            std::string data;
            ...
            if(mHeaders.find("Content-Type") != mHeaders.end() && mHeaders["Content-Type"] == "text/plain"){
