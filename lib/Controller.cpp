...
namespace Mongoose
{
    Controller::Controller(Server *server, Sessions *session):
        mServer(server),
        mSessions(session),
        mPrefix("")
    {
    ...
    Sessions *Controller::sessions() const
    {
        return mSessions;
    }

    void Controller::setSessions(Sessions *s)
    {
        mSessions = s;
    }

    bool Controller::isAuth(const std::shared_ptr<Request> &request, const std::shared_ptr<Response> &response)
    {
        
        return mSessions->isExits(request, response);
        
    }

    Session* Controller::setSessions(const std::shared_ptr<Request> &request, const std::shared_ptr<Response> &response){
        return mSessions->get(request, response);
    }
